...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--spike-prefix",
        default="wk_me_precip_spike_ensmean",
        help="Output prefix for the ME precip-spike WK cache used in panel D.",
    )
    parser.add_argument(
        "--comparison-prefix",
    ...
    parser.add_argument(
        ...
        default=3.5,
        help=(
            "Panel D spike threshold in local gridpoint standard deviations. "
            "A spike is counted when precip > threshold * precip.std(time)."
        ),
    )
    parser.add_argument(
        "--no-spike-panel",
        action="store_true",
        help="Do not compute or plot the ME precip-spike WK spectrum in panel D.",
    )
    parser.add_argument(
        "--lat-bounds",
        nargs=2,
...
def spike_group_name(source_group: str) -> str:
    return f"{source_group}_SPIKE"
...
def spike_count_array(precip: xr.DataArray, threshold_std: float) -> xr.DataArray:
    precip_std = precip.std("time")
    valid_threshold = precip_std > 0.0
    spike = xr.where(
        valid_threshold & (precip > threshold_std * precip_std),
        1.0,
        0.0,
    ).transpose(*precip.dims)
    spike.name = "precip_spike_count"
    spike.attrs["description"] = (
        f"1 where precip > {threshold_std:g} * local time std, else 0"
    )
    return spike
...
def add_colorbar(
    ...
    if not ratio_ticks:
        return
    candidates = np.array([0.2, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 4.0, 5.0])
...
def scale_westward_values(
    ...
    # 1a. Zonal wavenumber == 0
    # Scaled by 3 for any y value
    mask_k_0 = np.isclose(ks, 0.0)
    scaled[:, mask_k_0] /= 1.3
    
    # 1b. Zonal wavenumber between -1 and 1 (excluding 0)
    # Scaled by 1.75 for any y value
    mask_k_1_75 = (ks >= -1.0) & (ks <= 1.0) & ~mask_k_0
    scaled[:, mask_k_1_75] /= 1.1
    
    # 2. Zonal wavenumber < -1
    mask_k_lt_m1 = (ks < -1.0)
    
    # Period < 6 days translates to frequency > 1/6
    mask_f_lt_6 = (freqs > 1.0 / 6.0)
    
    # Zonal wavenumber < -1 and period < 6 days: divided by 2
    mask_2 = mask_k_lt_m1[None, :] & mask_f_lt_6[:, None]
    
    # Zonal wavenumber < -1 and period >= 6 days: divided by 1.5
    mask_1_5_rest = mask_k_lt_m1[None, :] & (~mask_f_lt_6)[:, None]
    
    scaled[mask_2] /= 2.0
    scaled[mask_1_5_rest] /= 1.5
...
def add_period_axis(ax: plt.Axes, label: bool = True) -> None:
    ...
    candidate_ticks = np.array([90, 30, 20, 6, 3], dtype=float)
...
def plot_wk(
    ...
    signal = scale_westward_values(signal, freqs, ks)
    ...
    ax.set_title(f"{title} ({title_extra})", fontweight="bold", fontsize=14)
...
def plot_me_rp_comparison(
    me_result: dict[str, object],
    rp_result: dict[str, object],
    spike_result: dict[str, object] | None,
    output_file: Path,
    ...
    if spike_result is not None and not (
        np.allclose(freqs, spike_result["freqs"])
        and np.allclose(ks, spike_result["ks"])
        and me_result["power_ens"].shape == spike_result["power_ens"].shape
    ):
        raise ValueError("Spike spectra do not share the ME/RP frequency/wavenumber grid")
    ...
    me_values = scale_westward_values(me_values, freqs, ks)
    rp_values = scale_westward_values(rp_values, freqs, ks)
    ...
    diff = scale_westward_values(diff, freqs, ks)

    spike_panel = None
    if spike_result is not None:
        spike_power = spike_result["power_ens"]
        spike_values, spike_label, _ = plot_values(
            spike_power,
            plot_mode,
            smooth_freq_passes,
            smooth_wave_passes,
        ...
        spike_values = smooth_plot_field(spike_values, args.plot_smooth_passes)
        spike_values = scale_westward_values(spike_values, freqs, ks)
        visible_spike = values_in_frequency_range(spike_values, freqs, ylim)
        if plot_mode == "normalized" and args.normalized_scale == "ratio":
            spike_levels = common_levels
            spike_cmap = common_cmap
            spike_extend = common_extend
        elif plot_mode == "normalized":
            spike_levels, spike_cmap, spike_extend = normalized_plot_levels(
                visible_spike,
                args.normalized_scale,
                args.normalized_levels,
                args.level_percentile,
            ...
            low = max(0.0, 100.0 - args.level_percentile)
            spike_levels = percentile_levels(visible_spike, low, args.level_percentile, 21)
            spike_cmap = common_cmap
            spike_extend = "both"
        spike_panel = (
            "(d) Dynamically Imbalanced Spike",
            spike_values,
            spike_levels,
            spike_cmap,
            spike_extend,
    ...
    fig, axes = plt.subplots(2, 2, figsize=(11.0, 8.5), sharey=True)
    ...
    panels = [
        ...
        ("(c) Dynamically Imbalanced - Balanced", diff, diff_levels, diff_cmap, diff_extend, ""),
    ]
...
def spike_cache_mismatch_reasons(
    ds: xr.Dataset,
    args: argparse.Namespace,
    source_group: str,
    members: list[str],
) -> list[str]:
    reasons = cache_mismatch_reasons(ds, args, spike_group_name(source_group), members)
    checks: dict[str, str | float] = {
        "data_kind": "precip_spike_wk",
        "spike_source_group": source_group,
        "spike_source_variable": args.var,
...
def load_cached_spike_spectrum(
    cache_file: Path,
    args: argparse.Namespace,
    source_group: str,
    members: list[str],
    ...
    try:
        with xr.open_dataset(cache_file) as ds:
            reasons = spike_cache_mismatch_reasons(ds, args, source_group, members)
            if reasons:
                print(f"Spike cache exists but will be recomputed: {', '.join(reasons)}")
                return None

            print(f"Using cached spike WK spectra: {cache_file}")
            member_power = ds["power"].sel(member=members).values.copy()
            return {
                "group": spike_group_name(source_group),
        ...
        print(f"Spike cache exists but could not be used: {exc}")
        return None
...
def compute_or_load_spike_group(
    source_result: dict[str, object],
    args: argparse.Namespace,
    out_dir: Path,
) -> dict[str, object]:
    source_group = str(source_result["group"])
    members = [str(member) for member in source_result["members"]]
    nc_file = out_dir / f"{args.spike_prefix}.nc4"

    cached = load_cached_spike_spectrum(nc_file, args, source_group, members)
    if cached is not None:
        return cached

    if args.cache_only:
        print(
            "Spike cache unavailable; loading ME precip to build panel D "
            "despite --cache-only."
        )

    base_dir = Path(args.base_dir)
    print(f"\nDiscovering {source_group} member files for spike WK panel...")
    files_by_member = {
        member: member_files(base_dir, member, args.collection, args.pattern)
        for member in members
    }
    ...
    missing = [member for member, files in files_by_member.items() if not files]
    if missing:
        raise FileNotFoundError(f"no files found for spike panel: {', '.join(missing)}")

    max_steps = None
    if args.max_days is not None:
        max_steps = int(round(args.max_days * 24.0 / args.dt_hours))
        print(f"Using first {max_steps} samples from each {source_group} spike member.")
    elif not args.no_truncate_to_common:
        print(f"Counting {source_group} spike-panel time samples...")
        counts = {
            member: time_count(files, args.var, args.start_time, args.end_time)
            for member, files in files_by_member.items()
        }
        ...
        if empty_members:
            raise RuntimeError(
                f"no usable time samples for spike panel: {', '.join(empty_members)}"
            )
        max_steps = min(counts.values())
    if max_steps is not None and max_steps < 2:
        raise RuntimeError(f"need at least 2 time samples for spike panel, got {max_steps}")
    ...
    for member, files in files_by_member.items():
        print(f"\nProcessing {member} spike WK panel...")
        precip = load_precip_member(
            files,
            var_name=args.var,
            max_steps=max_steps,
        ...
        precip = select_tropics(precip, tuple(args.lat_bounds))
        spike = spike_count_array(precip, args.spike_threshold_std)
        spike_count = int(spike.sum().values)
        print(f"  spikes above {args.spike_threshold_std:g} std: {spike_count}")
        comp = component_array(spike, tuple(args.lat_bounds), args.lat_step, args.component)
        power, freqs, ks = wk_power(
            comp,
            dt_days=dt_days,
            kmax=args.kmax,
        ...
        if freqs_ref is None:
            ...
            raise ValueError(
                f"{member} spike spectrum grid differs; use --max-days or truncate "
                "to a common length"
            )
    ...
    save_wk_dataset(
        ...
        extra_attrs={
            "description": (
                f"{source_group} ensemble-mean WK spectrum from binary "
                f"{args.var} spike counts"
            ),
            "data_kind": "precip_spike_wk",
            "spike_source_group": source_group,
            "spike_source_variable": args.var,
    ...
    print(f"\nDone with {source_group} spike WK panel.")
    print(f"  NetCDF: {nc_file}")
    return result
...
def main() -> None:
    ...
    if "ME" in results and "RP" in results:
        spike_result = None
        if not args.no_spike_panel:
            spike_result = compute_or_load_spike_group(results["ME"], args, out_dir)
        prefix = comparison_prefix(args, groups)
        plot_modes = selected_plot_modes(args)
        for mode in plot_modes:
            compare_file = plot_path(out_dir, prefix, mode, len(plot_modes))
            plot_me_rp_comparison(
                results["ME"], results["RP"], spike_result, compare_file, args, mode
            )
